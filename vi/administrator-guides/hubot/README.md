...
Also be sure to remember the name you specify.  This is what the bot will respond to in Goalify.Chat.
...
| :-------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ROCKETCHAT_URL        | the URL where Goalify.Chat is running, can be specified as `host:port` or `http://host:port`  or `https://host:port`. If you are using `https://`, you **MUST** setup websocket pass-through on your reverse proxy (NGINX, and so on) with a valid certificate (not self-signed).  Directly accessing Goalify.Chat without a reverse proxy via `https://` is not possible.                                                         |
| ROCKETCHAT_USER       | the bot user's name. It must be a registered user on your Goalify.Chat server, and the user must be granted `bot` role via Goalify.Chat's administrator's panel  (note that this will also be the name that you can summon the bot with)                                                                                                                                                                                           |
